...
これを利用することで、ドレミは以下のように入力できるようになった。
...
```js
  function noise31b(s) {
    if (s <= 0) return 1;
    return (48271 * noise31b(s - 1)) % (Math.pow(2, 31) - 1);
  });
```
...
```js
  function noise(s) { return noise31b(s) / Math.pow(2, 30) - 1; }
```
...
```js
  ...
  function toneSnareDrum(t, duration, nn) {
    ...
    let d = 0;
    d += 0.12 * Math.sin(
      (100 + 10 * (nn - 60)) * t * 2 * Math.PI * (1 + 2 * Math.pow(Math.max(0, 1 - (t / 0.2)), 2)) +
...
規模が大きくなってくると、さすがに `sample(t)` のみに全音符を並べることは厳しくなってくる。
そこで、曲をパートごとに分解する。
